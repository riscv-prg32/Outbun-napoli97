...
The hero car is a **white Fiat 500**, Abarth-style with its engine lid propped open. The other players — and the CPU rivals when fewer than four humans play — drive the other souped-up classics: **Fiat 126**, **Citroën Dyane** and **VW Maggiolino (Beetle)**. Each has its own top speed, acceleration and grip. The road is shared with Piaggio Apes full of lemons, Vespas and the blue SITA coach.
...
| | bytes |
|---|---:|
| code, tables and sprites (portable ABI image) | 43,168 |
| AUDIO block (6 tracks, 8 procedural voices) | 2,496 |
| header, metadata, colophon, icon, screenshot | 8,612 |
| **cartridge** | **54,276 / 65,536** |

The executable image uses 48,220 bytes of the 64 KiB cartridge RAM. Real-firmware QEMU captures are in [`release-artifacts/qemu/`](release-artifacts/qemu/). [docs/TECHNICAL_NOTES.md](docs/TECHNICAL_NOTES.md) explains how the art fits: a programmed 213-colour palette, row-group nibble-RLE sprites and procedural panoramas.
